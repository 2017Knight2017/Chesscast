...
def get_durations(pgn: str, initial_time: int, archetypes: tuple[str, str], archetypes_path="archetypes.json") -> list[float]:
	pgn_io = StringIO(pgn)
	...
	clocks = {chess.WHITE: initial_time, chess.BLACK: initial_time}
	...
	with chess.engine.SimpleEngine.popen_uci("../usr/games/stockfish") as eng:
		...
		for move in game.mainline_moves():
			...
			moves_to_control = (next_control_ply - moves_played) // 2
			...
			clocks[current_player] -= move_time
			if clocks[current_player] < 0:
				clocks[current_player] = 0
				
			moves_played += 1
			print(f"Полуход {moves_played}. Ход {'черных' if current_player else 'белых'}: {move_time:.1f} сек")
			board.push(move)
