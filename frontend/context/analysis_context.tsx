...
export function AnalysisProvider({ children }: { children: ReactNode }) {
	...
	const addMoveToTree = useCallback(
		(move: string, branchRootIndex: number, parentPath: number[] = []) => {
			setAnalysisTree((prevTree) => {
				...
				const existingMoveIndex = currentLevel.findIndex(
					(node) => node.m === move,
				);
	...
	const deleteBranch = useCallback(
		(branchRootIndex: number, path: number[]) => {
			...
			setAnalysisTree((prevTree) => {
				...
				for (let i = 0; i < path.length - 1; i++) {
					const idx = path[i];
					if (!currentLevel[idx] || !currentLevel[idx].s)
						return prevTree;
					currentLevel = currentLevel[idx].s!;
				}
	...
	const checkExistingAnalysis = useCallback(
		async (currentMatchId: string, userId: number): Promise<boolean> => {
			try {
				return await checkExistingAnalysisAction(
					currentMatchId,
					userId,
				);
			} catch (error) {
	...
	const loadAnalysis = useCallback(
		async (currentMatchId: string, userId: number) => {
			try {
...
export function useAnalysisState() {
	...
	if (context === undefined) {
		throw new Error(
			"useAnalysisState must be used within an AnalysisProvider",
		);
	}
	return context;
}
