...
export default function BroadcastPage() {
	...
	const inputClasses = "w-full p-3 rounded-sm bg-stone-900 border border-amber-900/30 focus:border-white-500 text-stone-100 outline-none transition-all disabled:opacity-50 disabled:cursor-not-allowed placeholder:text-stone-600";
	const labelClasses = `block ${isDisabled ? 'opacity-40' : 'opacity-100'} transition-opacity`;
	const titleLabelClasses = "text-xs font-bold uppercase tracking-wider text-amber-200/60 mb-2 block";
					...
					{activeTab === 1 && (
						<div className="space-y-8 animate-in fade-in duration-300">
							<label className="block">
								<span className={titleLabelClasses}>Broadcast Title</span>
								<input
									type="text"
							...
							<label className="block">
								<span className={titleLabelClasses}>Start Date & Time (Local)</span>
								<input
									type="datetime-local"
							...
							<div>
								<span className="text-xs font-bold uppercase tracking-wider text-stone-500 mb-4 block border-b border-white/5 pb-2">Time Control</span>
								<div className='grid gap-6 grid-cols-2'>
									<label className="block">
										<span className={titleLabelClasses}>Base Time (H:MM)</span>
										<input
											type="text"
											placeholder="1:30"
											className={inputClasses}
									...
									<label className="block">
										<span className={titleLabelClasses}>Increment (SEC)</span>
										<input
											type="number"
											className={inputClasses}
											value={timeIncrement}
					...
					{activeTab === 2 && (
						<div className="space-y-8 animate-in fade-in duration-300">
							<div className="grid gap-8 grid-cols-2">
								...
								/>
								<label className="block">
									<span className={titleLabelClasses}>White Archetype</span>
									<select
								...
								<label className="block">
									<span className={titleLabelClasses}>Black Archetype</span>
									<select
										className="w-full p-2 rounded bg-slate-800 border border-slate-700 focus:border-blue-500 outline-none"
										value={archetype2}
					...
					{activeTab === 3 && (
						<div className="space-y-8 animate-in fade-in duration-300">
							<div className="space-y-4 bg-black/20 p-6 rounded border border-white/5">
								...
								<div className={`grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-4 transition-all duration-300 ${isDisabled ? 'opacity-30 pointer-events-none' : ''}`}>
									<label className={labelClasses}>
										<span className={titleLabelClasses}>Move #</span>
										<input type="number" className={inputClasses} value={controlMove} onChange={(e) => setControlMove(parseInt(e.target.value))} />
									</label>
									<label className={labelClasses}>
										<span className={titleLabelClasses}>Bonus (Min)</span>
										<input type="number" className={inputClasses} value={bonusTimeMin} onChange={(e) => setBonusTimeMin(parseInt(e.target.value))} />
									</label>
									<label className={labelClasses}>
										<span className={titleLabelClasses}>New Inc (Sec)</span>
										<input type="number" className={inputClasses} value={newTimeIncrement} onChange={(e) => setNewTimeIncrement(parseInt(e.target.value) || 0)} />
									</label>
									<div className={`flex items-start gap-3 h-full pt-1 ${labelClasses}`}>
										...
										{isRepeatableControlMove && isControlMove ? (
											<div className="block flex-1">
												<span className={titleLabelClasses}>Next Control Move After</span>
												<input
													type="number"
													className={inputClasses}
											...
											<span className={titleLabelClasses}>Repeatable Control Move</span>
										)}
									</div>
								</div>
							</div>
								<label className="block">
									<span className={titleLabelClasses}>Initial PGN</span>
									<textarea
